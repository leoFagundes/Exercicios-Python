...
Sua equipe foi contratada para desenvolver este programa, utilizando a linguagem de programação Python. Para computar cada voto, 
a telefonista digitará um número, entre 1 e 23, correspondente ao número da camisa do jogador. Um número de jogador igual zero, 
indica que a votação foi encerrada. Se um número inválido for digitado, o programa deve ignorá-lo, mostrando uma breve mensagem de aviso, 
...
while True:
    escolha = int(input("Número do jogador (0 = [fim]): "))
    if escolha == 0:
        break
    elif escolha < 0 or escolha > 23:
...
print(f"\n{'Jogador':^15}{'Votos':^15}{'%':^15}")
for i in jogadores:
    print(f"{i:^15}{votos.count(i):^15}{f'{votos.count(i)/len(votos):.1%}':^15}")