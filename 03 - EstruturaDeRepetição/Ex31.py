...
while True:
    total = 0
    while True:
        valor = float(input("Valores: (Digite 0 para encerrar a compra) "))
        if valor  == 0:
            break
        else:
            total += valor
    ...
    while True:
        din = float(input("Qual o valor em dinheiro para o pagamento? "))
        if din < total:
            print("\ninforme uma nota mais alta.")
            continue
        else:
            break
    print(f"O troco é de R${din-total}")